 ...
 Logger logger = LoggerFactory.getLogger(AlipayNotify.class);
	...
	*/
	...
	if(signVerified) {//验证成功
        ...
        logger.info("total_amount="+total_amount+"out_trade_no="+out_trade_no+"seller_id="+seller_id+"app_id="+app_id);
        boolean  checkData=orderService.checkRespData(out_trade_no, total_amount, seller_id, app_id);
        if(checkData){
            if(trade_status.equals("TRADE_FINISHED")){
                ...
                //如果没有做过处理，根据订单号（out_trade_no）在商户网站的订单系统中查到该笔订单的详细，并执行商户的业务程序
                //如果有做过处理，不执行商户的业务程序
                logger.error("支付宝支付成功");
                if(orderService.alipayNotify(trade_status,null,out_trade_no,trade_no)){
